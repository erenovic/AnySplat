    ...
    def forward(
        ...
        for level in lod_rendering.keys():
            ...
            resized_image = F.interpolate(
                image.clone().flatten(0, 1), size=(_h, _w), mode="bilinear", align_corners=False
            )
            ...
            loss += (
                torch.nan_to_num(level_mse_loss, nan=0.0, posinf=0.0, neginf=0.0) * self.cfg.mse_weight
                + torch.nan_to_num(level_lpips_loss, nan=0.0, posinf=0.0, neginf=0.0) * self.cfg.lpips_weight
            )
