...
class ViewSamplerRank(ViewSampler[ViewSamplerRankCfg]):
    ...
    def num_ctxt_gap_mapping_target(self) -> dict:
        ...
        for num_ctxt in range(2, self.cfg.num_context_views + 1):
            mapping[num_ctxt] = [
                max(num_ctxt * 2, self.cfg.num_target_views + num_ctxt),
                max(
                    self.cfg.num_target_views + num_ctxt,
                    min(num_ctxt**2, self.cfg.max_distance_between_context_views),
                ),
            ]
        return mapping
    ...
    def num_ctxt_gap_mapping(self) -> dict:
        ...
        for num_ctxt in range(2, self.cfg.num_context_views + 1):
            mapping[num_ctxt] = [
                min(num_ctxt * 3, self.cfg.min_distance_between_context_views),
                min(max(num_ctxt * 5, num_ctxt**2), self.cfg.max_distance_between_context_views),
            ]
        return mapping
