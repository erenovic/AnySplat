...
class DatasetCo3d(Dataset):
    ...
    def getitem(self, index: int, num_context_views: int, patchsize: tuple) -> dict:
        ...
        target_indices = torch.tensor(
            random.sample(remaining_indices.tolist(), self.view_sampler.num_target_views)
        )
        ...
        if self.cfg.make_baseline_1:
            ...
            if scale < self.cfg.baseline_min or scale > self.cfg.baseline_max:
                print(f"Skipped {scene_id} because of baseline out of range: {scale:.6f}")
        ...
        if (
            ...
            or torch.isnan(example["context"]["pts3d"]).any()
            or torch.isinf(example["context"]["pts3d"]).any()
            ...
            or torch.isnan(example["target"]["pts3d"]).any()
            or torch.isinf(example["target"]["pts3d"]).any()
