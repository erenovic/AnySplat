...
class AttnBasedAppearanceHead(nn.Module):
    ...
    def __init__(
        ...
        self.attn_processor = AppearanceTransformer(
            num_layers=4, attn_dim=dim_tokens[0] * 2, head_dim=feature_dim
        )
        ...
        self.token_decoder = nn.Sequential(
            ...
            nn.Linear(
                dim_tokens[0] * 2,
                self.num_channels * (self.patch_size[0] ** 2),
                bias=False,
            ),
        )
    ...
    def img_pts_tokenizer(self, imgs, pts3d):
        ...
        z_median = torch.median(torch.norm(pts3d, dim=-1, keepdim=True), dim=2, keepdim=True)[
            0
        ]  # (B, V, 1, 1)
    ...
    def forward(self, x, depths, imgs, img_info, conf=None):
        ...
        # Hook decoder onto 4 layers from specified ViT layers
        layer_tokens = [
            rearrange(x[hook].detach(), "(b v) l d -> b (v l) d", b=B, v=V) for hook in self.hooks
        ]
        ...
        gaussians = rearrange(
            gaussian_params,
            "b (v hh ww) (ph pw d) -> b (v hh ph ww pw) d",
            v=V,
            hh=hh,
            ww=ww,
            ph=patch_size[0],
